...
class knnClassifier:
    ...
    ''' 
        ...
        The classifier will classify the test set into separate classes by comparing each instance with the k closest neighbours.

        (Note: The number of classes do not matter)
    ...
    def _get_k_nearest_neighbours(self, test_instance, k):
        ...
        for x in range(len(self.training_set_)):
            # ----- find the euclidean distance of each data point and add the values to distances
...
if __name__ == '__main__':
    ...
    ''' 
        ...
        The classifier is supposed to classify the test set into 3 classes by comparing the instance with the three (k = 3) closest neighbours.

    '''
    classifier = knnClassifier(filename = 'iris.data', num_attributes = 4, split_ratio = 0.65)

    # ----- The iris dataset consists of data of 3 different classes.
