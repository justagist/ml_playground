...
def plot_iteration(first_param_grid, sampled_params, sampled_loss, first_iter=0, alpha=1e-5,
                   ...
                   param_dims_to_plot=[0, 1], filepath="figs", optimum=None):
    """ plot_iteration
    ...
    for i in range(first_iter, sampled_params.shape[0] - 1):
        ...
        if second_param_grid is None:
            ...
            ei = -1 * bayesian_optimisation.expected_improvement(first_param_grid, model, sampled_loss[:(i + 1)],
            ...
            ei = -1 * bayesian_optimisation.expected_improvement(param_grid, model, sampled_loss[:(i + 1)],
